                ...
                if ($video_model->save()) {
	            	...
	            	if($request->hasFile('default_image')) {
	                	...
	                	if (Setting::get('ffmpeg_installed') == FFMPEG_INSTALLED) {
	                		...
	                		$path = base_path('public').'/uploads/images/385x225';

	                		$default_image_input_path = base_path('public').'/uploads/images/'.get_video_end($video_model->default_image);

	                		$default_image_output_path = base_path('public').'/uploads/images/385x225/'.get_video_end($video_model->default_image);
...
//	                            dispatch(new StreamviewCompressVideo($inputFile, $local_url, MAIN_VIDEO, $video_model->id, $file_name, $request->send_notification , $video_status , $request->admin_video_id));
	                ...
	                if (Setting::get('ffmpeg_installed') == FFMPEG_INSTALLED) {
	                	...
	                	$path = base_path('public').'/uploads/gifs';
			            ...
			            $FFmpeg
			                ->setImage('image2')
			                ->setFrameRate(1)
			                ->input( base_path('public')."/uploads/images/video_{$video_model->id}_%03d.png")
			                ->setAspectRatio("4:2")
			                ->frameRate(50)
			                ->output(base_path('public')."/uploads/gifs/video_{$video_model->id}.gif")
			                ->ready();
